...
void DoFileOpen(HWND hwnd)
{
	...
	if(GetOpenFileName(&ofn))
	{
		...
		{
			SendDlgItemMessage(hwnd, IDC_MAIN_STATUS, SB_SETTEXT, 0, (LPARAM)"Відкрито:");
...
void DoFileSave(HWND hwnd)
{
	...
	if(GetSaveFileName(&ofn))
	{
		...
		{
			SendDlgItemMessage(hwnd, IDC_MAIN_STATUS, SB_SETTEXT, 0, (LPARAM)"Збережено:");
			SendDlgItemMessage(hwnd, IDC_MAIN_STATUS, SB_SETTEXT, 1, (LPARAM)szFileName);
		}
	}
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch(msg)
	{
		case WM_CREATE:
		{
			...
			SendMessage(hStatus, SB_SETTEXT, 0, (LPARAM)"Готово");
		}
		break;
		case WM_SIZE:
		{
